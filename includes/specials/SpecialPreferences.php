...
class PreferencesForm {
	...
	var $mReset, $mPosted, $mToggles, $mSearchNs, $mRealName, $mTwitter, $mImageSize;
	...
	function __construct( &$request ) {
		...
		$this->mTwitter = $request->getVal('wpTwitter');
	...
	function savePreferences() {
		...
		$wgUser->setTwitter($this->mTwitter);
	...
	function resetPrefs() {
		...
		$this->mTwitter = $wgUser->getTwitter();
	...
	function mainPrefsForm( $status , $message = '' ) {
		...
		$wgOut->addHTML(
				$this->tableRow(
					Xml::label( 'Twitter Handle:', 'wpTwitter' ),
					Xml::input( 'wpTwitter', 25, $this->mTwitter, array( 'id' => 'wpTwitter' ) ),
					Xml::tags('div', array( 'class' => 'prefsectiontip' ),
						"Your Twitter Handle is optional, but it allows your Twitter feed to be shown on the wiki."
					)
				)
			);

...
class EditMember extends SpecialPage{
	...
	function generateMain(){
	    ...
	    foreach($allPeople as $person){
	        ...
	        $wgOut->addHTML("<option class='".implode(" ", $projs)."' id='".str_replace(".", "", $person->getName())."'>".str_replace(".", " ", $person->getNameForForms())."</option>\n");
	    }
	...
	function generateEditMemberFormHTML($wgOut){
        ...
        if($me->isRoleAtLeast(STAFF)){
            $wgOut->addHTML("<li><a href='#tabs-3'>Project Leadership</a></li>
                             <li><a href='#tabs-5'>Project Manager</a></li>
                             <li><a href='#tabs-4'>Theme Leadership</a></li>");
        }
        $wgOut->addHTML("
                    </ul>");
		    ...
		    if($user->isRoleAtLeast($role)){
		        ...
		        if($role == HQP){
		            $boxes .= " /> $role<div style='display:none; padding-left:30px;'>
		                                    ...
		                                    <tr>
		                                        <td colspan='2'>Additional Comments:<br /><textarea name='r_comment[".$role."]' cols='15' rows='4' ></textarea></td>
		                                    </tr>
		                                </table>
		                                </fieldset>
		                                </div><br />";
		            ...
		            $boxes .= " /> $role<div style='display:none; padding-left:30px;'><fieldset><legend>Reasoning</legend><p>Date&nbsp;Effective:<input type='text' class='datepicker' id='datepicker{$role}' name='r_datepicker[$role]' /></p>Additional Comments:<br /><textarea name='r_comment[".$role."]' cols='15' rows='4' ></textarea></fieldset></div><br />";
		        }
		    }
		    else{
		...
		foreach($pArray as $project){
		    ...
		    if(!$skip){
	            ...
	            if($person->isMemberOf(Project::newFromName($project))){
	                $wgOut->addHTML("<td style='min-width:150px;' valign='top'><input type='checkbox' name='p_wpNS[]' value='$project' checked='checked' class='already' onChange='addComment(this, false);' /> $project<div style='display:none; padding-left:30px;'><fieldset><legend>Reasoning</legend><p>Date Effective:<input type='text' class='datepicker' id='datepicker{$project}' name='p_datepicker[$project]' /></p>Additional Comments:<br /><textarea name='p_comment[$project]' cols='15' rows='4' ></textarea></fielset></div><br /></td>\n");
	            }
	            else {
	                $wgOut->addHTML("<td style='min-width:150px;' valign='top'><input type='checkbox' name='p_wpNS[]' value='$project' /> $project</td>\n");
	            }
	            $i++;
	        }
	        else{
	...
	function generatePLFormHTML($wgOut){
		...
		$i = 0;
		$wgOut->addHTML("<table border='0' cellspacing='2'>");
		$leadProjects = $person->getLeadProjects();
		foreach($pArray as $project){
	        if($i % 3 == 0){
	            $wgOut->addHTML("</tr><tr>\n");
            }
            $isLead = false;
            foreach($leadProjects as $proj){
                if($proj->getName() == $project){
                    $isLead = true;
                    break;
                }
            }
            if($isLead && !$person->isProjectManager()){
                $wgOut->addHTML("<td style='min-width:150px;' valign='top'><input type='checkbox' name='pl[]' value='$project' checked='checked' class='already' onChange='addComment(this, false);' /> $project<div style='display:none; padding-left:30px;'><fieldset><legend>Reasoning</legend><p>Date Effective:<input type='text' class='datepicker' id='pl_datepicker{$project}' name='pl_datepicker[$project]' /></p>Additional Comments:<br /><textarea name='pl_comment[$project]' cols='15' rows='4' ></textarea></fielset></div><br /></td>\n");
            }
            else {
                $wgOut->addHTML("<td style='min-width:150px;' valign='top'><input type='checkbox' name='pl[]' value='$project' /> $project</td>\n");
            }
            $i++;
		}
		$wgOut->addHTML("</tr></table>");
		...
		$i = 0;
		$wgOut->addHTML("<table border='0' cellspacing='2'>");
		$coLeadProjects = $person->getCoLeadProjects();
		foreach($pArray as $project){
	        if($i % 3 == 0){
	            $wgOut->addHTML("</tr><tr>\n");
            }
            $isLead = false;
            foreach($coLeadProjects as $proj){
                if($proj->getName() == $project){
                    $isLead = true;
                    break;
                }
            ...
            if($isLead && !$person->isProjectManager()){
                $wgOut->addHTML("<td style='min-width:150px;' valign='top'><input type='checkbox' name='copl[]' value='$project' checked='checked' class='already' onChange='addComment(this, false);' /> $project<div style='display:none; padding-left:30px;'><fieldset><legend>Reasoning</legend><p>Date Effective:<input type='text' class='datepicker' id='copl_datepicker{$project}' name='copl_datepicker[$project]' /></p>Additional Comments:<br /><textarea name='copl_comment[$project]' cols='15' rows='4' ></textarea></fielset></div><br /></td>\n");
            }
            else {
                $wgOut->addHTML("<td style='min-width:150px;' valign='top'><input type='checkbox' name='copl[]' value='$project' /> $project</td>\n");
            }
            $i++;
		}
		$wgOut->addHTML("</tr></table>");
	...
	function generateTLFormHTML($wgOut){
		...
		for($themeId = 1; $themeId <= 5; $themeId++){
            ...
            if($isLead){
                $wgOut->addHTML("<tr><td style='min-width:150px;' valign='top'><input type='checkbox' name='tl[]' value='$themeId' checked='checked' class='already' onChange='addComment(this, false);' /> $themeId - ".Project::getThemeName($themeId)."<div style='display:none; padding-left:30px;'><fieldset><legend>Reasoning</legend><p>Date Effective:<input type='text' class='datepicker' id='tl_datepicker{$themeId}' name='tl_datepicker[$themeId]' /></p>Additional Comments:<br /><textarea name='tl_comment[$theme]' cols='15' rows='4' ></textarea></fielset></div><br /></td></tr>\n");
            }
            else {
                $wgOut->addHTML("<tr><td style='min-width:150px;' valign='top'><input type='checkbox' name='tl[]' value='$themeId' /> $themeId - ".Project::getThemeName($themeId)."</td></tr>\n");
            }
		}
		$wgOut->addHTML("</table>");
		...
		for($themeId = 1; $themeId <= 5; $themeId++){
            ...
            if($isLead){
                $wgOut->addHTML("<tr><td style='min-width:150px;' valign='top'><input type='checkbox' name='cotl[]' value='$themeId' checked='checked' class='already' onChange='addComment(this, false);' /> $themeId - ".Project::getThemeName($themeId)."<div style='display:none; padding-left:30px;'><fieldset><legend>Reasoning</legend><p>Date Effective:<input type='text' class='datepicker' id='cotl_datepicker{$themeId}' name='cotl_datepicker[$themeId]' /></p>Additional Comments:<br /><textarea name='cotl_comment[$theme]' cols='15' rows='4' ></textarea></fielset></div><br /></td></tr>\n");
            }
            else {
                $wgOut->addHTML("<tr><td style='min-width:150px;' valign='top'><input type='checkbox' name='cotl[]' value='$themeId' /> $themeId - ".Project::getThemeName($themeId)."</td></tr>\n");
            }
		}
		$wgOut->addHTML("</table>");
	}
	...
	function generatePMFormHTML($wgOut){
		...
		foreach($pArray as $project){
            ...
            if($person->managementOf($project)){
                $wgOut->addHTML("<td style='min-width:150px;' valign='top'><input type='checkbox' name='pm[]' value='$project' checked='checked' class='already' onChange='addComment(this, false);' /> $project<div style='display:none; padding-left:30px;'><fieldset><legend>Reasoning</legend><p>Date Effective:<input type='text' class='datepicker' id='pm_datepicker{$project}' name='pm_datepicker[$project]' /></p>Additional Comments:<br /><textarea name='pm_comment[$project]' cols='15' rows='4' ></textarea></fielset></div><br /></td>\n");
            }
            else {
                $wgOut->addHTML("<td style='min-width:150px;' valign='top'><input type='checkbox' name='pm[]' value='$project' /> $project</td>\n");
            }
            $i++;
		}
		$wgOut->addHTML("</tr></table>");

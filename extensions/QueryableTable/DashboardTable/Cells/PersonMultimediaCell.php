<?php
    ...
    class PersonMultimediaCell extends DashboardCell {
        ...
        function PersonMultimediaCell($cellType, $params, $cellValue, $rowN, $colN, $table){
            ...
            if(isset($params[2])){
                $person = $table->obj;
                $project = Project::newFromId($params[2]);
                if($project != null && $project->getName() != null){
                    $this->obj = $project;
                    $multimedia = $person->getMultimedia();
                    $values = array();
...
EOF;
        ...
        function detailsRow($item){
            ...
            foreach($people as $p){
                $peoples[] = "<a href='{$p->getUrl()}' target='_blank'><b>{$p->getNameForForms()}</b></a>";
            }
            $details = "<td style='white-space:nowrap;text-align:center;'>{$date} </td><td>".implode(", ", $projs)."<br /></td><td>".implode(", ", $peoples)." </td><td><a href='{$media->getUrl()}' target='_blank'><i>{$media->getTitle()}</i></a></td>";
            return $details;
        }

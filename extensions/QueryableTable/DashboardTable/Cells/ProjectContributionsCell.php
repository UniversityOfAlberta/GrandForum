...
EOF;
        ...
        function detailsRow($item){
            ...
            $details = "<td style='white-space:nowrap;text-align:center;'>{$date} </td><td style='text-align:right;'>\$".number_format($contribution->getTotal())." </td><td>".implode(", ", $projs)."<br /></td><td>".implode(", ", $parts)."</td><td><a href='{$contribution->getUrl()}' target='_blank'>{$contribution->getName()}</a><br /></td>";
            return $details;
        }

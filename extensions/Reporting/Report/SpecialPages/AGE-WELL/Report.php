...
class Report extends AbstractReport {
    ...
    static function createSubTabs(&$tabs){
        ...
        if($person->isRole(HQP)){
            $selected = @($wgTitle->getText() == "Report" && ($_GET['report'] == "AccessApplication072017")) ? "selected" : false;
            $tabs["Applications"]['subtabs'][] = TabUtils::createSubTab("ACCESS Application", "{$url}AccessApplication072017", $selected);
        }
        if($person->isRole(HQP) || $person->isRole(HQP.'-Candidate')){
        ...
        if($person->isRole(NI) || $person->isRole(NI.'-Candidate') ||
           $person->isRole(EXTERNAL) || $person->isRole(EXTERNAL.'-Candidate')){
            $selected = @($wgTitle->getText() == "Report" && $_GET['report'] == "SIPAccelerator") ? "selected" : false;
            $tabs["Applications"]['subtabs'][] = TabUtils::createSubTab("SIP Accelerator", "{$url}SIPAccelerator", $selected);
        ...
        if($person->isRole(PL) || $person->isRole(TL) || $person->isRole(TC) || $person->isRole(PS)){
            ...
            foreach($projects as $project){
                if($project->getType() != 'Administrative'){
                    ...
                    if($person->isRole(PL, $project) || $person->isRole(TL, $project) || $person->isRole(TC, $project)){

	...
	function userCanExecute($user){
        ...
        if($config->getValue('networkFullName') == "AVOID Australia" &&
            !($me->isRoleAtLeast(STAFF) ||
              ...
              $me->isRole("GroupC") && !$me->isRoleOn("GroupC", date('Y-m-d', time() - 86400*30.5*6)) || // Allow C until 6 months
	...
	function execute($par){
        ...
        foreach($categories as $category){
            ...
            if($category->videos > 0){
                $wgOut->addHTML("<a id='module{$category->id}' class='module' title='".showLanguage($category->title, $category->titleFr)."' href='{$url}'>

    ...
    function execute($par){
        ...
        foreach(self::$frailtyLabels as $i => $labels){
            @$wgOut->addHTML("<tr>
	                            ...
	                            <td style='white-space:nowrap;'>".count($frailty[$i])." (".number_format(count($frailty[$i])/max(1, $nIntake)*100, 1).")</td>
	                            <td>".number_format(avg($frailty[$i]), 2)."</td>
	                            <td>".number_format(median($frailty[$i]), 2)."</td>
	                            <td>".number_format(stdev($frailty[$i]), 2)."</td>
	                            <td style='white-space:nowrap;'>".count($frailty6[$i])." (".number_format(count($frailty6[$i])/max(1, $n6Month)*100, 1).")</td>
	                            <td>".number_format(avg($frailty6[$i]), 2)."</td>
	                            <td>".number_format(median($frailty6[$i]), 2)."</td>
	                            <td>".number_format(stdev($frailty6[$i]), 2)."</td>
	                            <td>".number_format(avg($means6[$i]), 4)."</td>
	                            <td>".number_format(pow(avg($means6[$i]), 2), 5)."</td>
	                            ...
	                            <td>".number_format(avg($means12[$i]), 4)."</td>
	                            <td>".number_format(pow(avg($means12[$i]), 2), 5)."</td>
	                        </tr>");
        }
        @$wgOut->addHTML("
                </tbody>
        ...
        @$wgOut->addHTML("
            ...
            <div>
                ...
                <h3>Most Improved</h3>
                <table class='wikitable'><tr><th>Name</th><th>ID</th><th>Delta</th><th>Last Completed</th><th>In Person</th></tr>");
                $count = 0;
                foreach($deltaImproved as $key => $delta){
                    if($count < 15 && $delta > 0){
                        $person = Person::newFromId($key);
                        $inPerson = (count($person->getRelations('Assesses', true, true)) >= 1) ? "Yes" : "No";
                        $lastCompleted = "";
                        ...
                        $wgOut->addHTML("<tr><td>{$person->getName()}</td><td>{$person->getId()}</td><td>+{$delta}</td><td>{$lastCompleted}</td><td>{$inPerson}</td></tr>");
                    }
                    $count++;
                }
                $wgOut->addHTML("

...
class TemplateReport extends AbstractReport{
    ...
    static function createSubTabs(&$tabs){
        ...
        if($person->isLoggedIn() && $person instanceof FullPerson){
            ...
            // Chair Table
            if(($person->inFaculty() && ($person->isRole(CHAIR) || $person->isRoleDuring(CHAIR, REPORTING_CYCLE_START, REPORTING_CYCLE_END) || $person->isRole(EA) || $person->isRole(ACHAIR))) || $person->isRole("Chair ".getFaculty())){
                $selected = @($wgTitle->getText() == "Report" && ($_GET['report'] == "ChairTable")) ? "selected" : false;

...
class DepartmentPeopleReportItemSet extends ReportItemSet {
    ...
    function getData(){
        ...
        if($me->getName() != "Christopher.Sturdy" && !$me->isRole(CHAIR) && !$me->isRole(CHAIR." ".getFaculty()) && !$me->isRole(EA) && 
                                                     !$me->isRoleDuring(CHAIR, REPORTING_CYCLE_START, REPORTING_CYCLE_END) &&
                                                     !$me->isRoleDuring(CHAIR." ".getFaculty(), REPORTING_CYCLE_START, REPORTING_CYCLE_END)){
        ...
        foreach($allPeople as $person){
            ...
            if(($dept == "") || 
               ...
               ($found)){
                ...
                if(($person->isRoleDuring(DEAN, REPORTING_CYCLE_START, REPORTING_CYCLE_END) || $person->isRole(DEAN)) && !$person->isSubRole("CR")){

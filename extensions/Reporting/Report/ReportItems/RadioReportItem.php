	...
	function render(){
		...
		if($orientation == 'table'){
			...
			if($table_type == 'header'){
				$output .= "<table cellspacing='0' class='wikitable' cellpadding='2'><tr><th></th>";
				...
				foreach($options as $i => $option){
					...
					$option = str_replace("'", "&#39;", $option);
					$output .= "<td style='text-align: center;'><input style='vertical-align:top;display:table-cell;transform-origin:top;' type='radio' name='{$this->getPostId()}' value='{$option}' $checked /></td>";
				}
				$output .= "</tr>";
			}
				...
				if(!is_array($option)){
					...
					if(count($labels) == count($options)){
						...
						if($orientation == 'horizontal'){
							$items[] = "<input style='vertical-align:top;display:table-cell;transform-origin:top;' type='radio' name='{$this->getPostId()}' value='{$option}' $checked />&nbsp;{$labels[$i]}";
						}

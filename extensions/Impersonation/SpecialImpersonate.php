...
class Impersonate extends SpecialPage {
	...
	function userCanExecute($user){
        ...
        return ($person->isRoleAtLeast(MANAGER) || $person->isRole(SD) || count($person->getDelegates()) > 0);
    }
	...
	function execute($par){
	    ...
	    $allPeople = array();
	    if($user->isRoleAtLeast(MANAGER) || $user->isRole(SD)){
	...
	static function createDelegateLink(&$toolbox){
        ...
        else if(!$wgImpersonating && !$wgDelegating && ($me->isRoleAtLeast(MANAGER) || $me->isRole(SD))){

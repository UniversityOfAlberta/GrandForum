...
class CCVExport extends SpecialPage {
   ...
   //            </script>");
    ...
    #$wgOut->addHTML("<p>User ID: {$wgUser->getId()}<p>");
    ...
    foreach($section as $item){
      if(
        ...
        if ((string)$pub_type->type !== ''){
          $field = $ccv_item->addChild("field");
          $field->addAttribute('jjjjjid', $pub_type['ccv_id']);

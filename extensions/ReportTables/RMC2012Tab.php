...
EOF;
    ...
    function showBudgetTableFor($type){
        ...
        if($type == PNI || $type == CNI){
            ...
            foreach(Project::getAllProjectsDuring("2011-01-01", "2011-12-31") as $project){
    ...
    function showProjectProductivity() {
        ...
        $projects = Project::getAllProjectsDuring("2011-01-01", "2011-12-31");
        $chunk = "
...
";
        ...
        foreach ($projects as $project) {
            ...
            foreach ($positions as $pos) {
                $hqps = Dashboard::getHQP($project, 0, $pos, "2011-01-01 00:00:00", "2012-01-01 00:00:00");
                $hqp_num = count($hqps);
...
EOF;
            ...
            // Publications.
            $papers = Dashboard::getPapers($project, '', "Publication", "2011-01-01 00:00:00", "2012-01-01 00:00:00");
...
EOF;
            ...
            // Artifacts.
            $papers = Dashboard::getPapers($project, '', "Artifact", "2011-01-01 00:00:00", "2012-01-01 00:00:00");
            $paper_num = count($papers);
            $paper_details = Dashboard::paperDetails($papers);
...
EOF;
            ...
            // Contributions
            $contributions = Dashboard::getContributions($project, '', '2011');
...
";
        ...
        foreach (array($pnis, $cnis) as $groups) {
            foreach ($groups as $person) {
                ...
                // HQP stuff.
                $hqps = Dashboard::getHQP(0, $person, 'all', "2011-01-01 00:00:00", "2012-01-01 00:00:00");
                foreach ($positions as $pos) {
...
EOF;
                ...
                // Publications.
                $papers = Dashboard::getPapers(0, $person, "Publication", "2011-01-01 00:00:00", "2012-01-01 00:00:00");
                $paper_num = count($papers);
                $paper_details = Dashboard::paperDetails($papers);
...
EOF;
                ...
                // Artifacts.
                $papers = Dashboard::getPapers(0, $person, "Artifact", "2011-01-01 00:00:00", "2012-01-01 00:00:00");
                $paper_num = count($papers);
                $paper_details = Dashboard::paperDetails($papers);
...
EOF;
                ...
                // Contributions
                $contributions = Dashboard::getContributions(0, $person, '2011');
                $contribution_num = count($contributions);
                $contribution_sum = Dashboard::contributionSum($contributions);
                $contribution_details = Dashboard::contributionDetailsXLS($contributions);
                $chunk .= "<td align='right'>{$contribution_num}</td>";
...
EOF;
    ...
    function getUniContributionStats(){
        ...
        foreach ($nis as $hqp){
            ...
            $contributions = Dashboard::getContributions(0, $hqp, '2011');

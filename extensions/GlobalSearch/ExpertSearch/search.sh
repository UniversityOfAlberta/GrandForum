...
./CommunityBasedEFPaper $queryFile "Index/indexFile/profiles_index.key" $klConfigFile $klOutFile "Clustering/output/papers-clusters.csv" $assocFile "Index/indexFile/publications_clusters_index.key" "all" $numProfile "0.9" "0.5" "Output/TopExperts.out" "Constrained" "10" "/dev/stdout" "0.6" "0.6" "TopFinalExperts.out" $numQueries

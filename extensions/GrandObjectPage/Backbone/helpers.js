...
HTML.TextArea = function(view, attr, options, model){
    ...
    el.setAttribute('name', HTML.Name(attr) + model.cid);
    el.innerHTML = HTML.Value(model, attr);
    view.events['change textarea[name=' + HTML.Name(attr) + model.cid + ']'] = function(e){
        if(attr.indexOf('.') != -1){
            var elems = attr.split(".");
            var recurse = function(data, depth) {
                if (depth < elems.length) {
    ...
    view.undelegate('change', 'textarea[name=' + HTML.Name(attr) + model.cid + ']');
    view.delegate('change', 'textarea[name=' + HTML.Name(attr) + model.cid + ']', view.events['change textarea[name=' + HTML.Name(attr) + model.cid + ']']);
    return el.outerHTML;
}

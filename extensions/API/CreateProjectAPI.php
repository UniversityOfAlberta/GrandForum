...
class CreateProjectAPI extends API{
    ...
    function ProjectDescriptionAPI(){
	    ...
	    $this->addPOST("proposed",true,"Whether or not this project should be a proposed project or not (true | false)","true");
    ...
    function processParams($params){
        ...
        $_POST['proposed'] = @mysql_real_escape_string($_POST['proposed']);
	...
	function doAction($noEcho=false){
	    ...
	    $proposed = (isset($_POST['proposed']) && $_POST['proposed'] == "true") ? '1' : '0';
	    $themes = @"{$theme1}\n{$theme2}\n{$theme3}\n{$theme4}\n{$theme5}";
	    ...
	    $sql = "INSERT INTO `grand_project` (`id`,`name`,`proposed`)
	            VALUES ('{$nsId}','{$_POST['acronym']}','{$proposed}')";
	    DBFunctions::execSQL($sql, true);
	    $sql = "INSERT INTO `grand_project_evolution` (`project_id`,`new_id`,`action`)
	            VALUES ('-1','{$nsId}','CREATE')";
	    DBFunctions::execSQL($sql, true);
	    Project::$cache = array();

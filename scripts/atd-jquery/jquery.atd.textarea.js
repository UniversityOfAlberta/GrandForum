...
var AtD = 
{
	rpc : wgServer+wgScriptPath+'/index.php?action=api.getError&last=false', /* see the proxy.php that came with the AtD/TinyMCE plugin */
	rpc_css : 'http://162.246.157.221/index.php?action=api.getError&encode=true&last=false', /* you may use this, but be nice! */
	rpc_css_lang : 'en',
	api_key : '',
	i18n : {},
	listener : {}
...
AtD.check = function(container_id, callback_f) {
	...
	jQuery.ajax({
		...
		success : function(data) {
			...
			var count = AtD.processXML(container_id, xml);
			console.log(xml);	
			console.log("should've highlighted here" + container_id);
			if (AtD.callback_f != undefined && AtD.callback_f.ready != undefined)
				AtD.callback_f.ready(count);
